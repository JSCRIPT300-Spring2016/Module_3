...
function filterNonEmptySched(value) {
  return (value.schedule.length > 0);
}

var filterTrucksByDay = function (day) {
  var trucksByDay = [];
  
  // Get list of trucks that have non-empty schedules.
  var trucksNonEmptySched = foodTrucks.filter(filterNonEmptySched);

  // Loop through each food truck and search for the specified day.
  for (var i = 0; i < trucksNonEmptySched.length; i++) {
    if ((trucksNonEmptySched[i].schedule.indexOf(day)) >= 0) {
      trucksByDay.push(trucksNonEmptySched[i]);
    }
  }
...
/*
* return an object from the module with a single method on it:
* filterTrucksByDay that method should take a single parameter that
* represents the day to filter on.  Use the built-in filter() method to
* return all trucks that have the day in their schedule.
*/
